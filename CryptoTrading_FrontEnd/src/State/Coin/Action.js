...
export const fetchMarketChart = ({ coinId, days, jwt }) => async (dispatch) => {
	...
	try {
		const response = await api.get(`/coins/${coinId}/chart?days=${days}`, { headers: { Authorization: `Bearer ${jwt}` } });
...
export const fetchCoinDetails = ({coinId, jwt}) => async (dispatch) => {
	...
	try {
		const response = await api.get(`/coins/details/${coinId}`, { headers: { Authorization: `Bearer ${jwt}` } });
